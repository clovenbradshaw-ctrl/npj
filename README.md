...
| `app/feedback.js` | **reader contributions** — a span suggestion/comment (`scope:"span"`) on any selected words, OR a contribution on the **whole article** (`scope:"article"` — a written proposal/correction, read like a PR description); stored as EVA events in the article's folder + a local mirror; an editor **merges** a span edit (apply + commit a REC) or resolves a whole-article proposal by hand |
| `app/SuggestionRail.jsx` | the review surface — PR-shaped cards (diff, rationale, 👍, threaded replies) with **Merge / Decline** for editors, plus the composer (span edit, span comment, or whole-article contribution) |
