...
| `app/MergeDashboard.jsx` | the **site-wide merge-requests dashboard** (its own nav tab) — folds every article's feedback log into one filterable table of branches (open/all/mine, public/private, search), with counts; visibility-aware. "Open ↗" jumps into the piece with the rail up to preview/merge/ignore; admins can ignore inline |
