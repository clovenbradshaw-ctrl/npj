...
| `app/feedback/authorship.js` | the Newsroom's **Authors** mode — colors each collaborator's own words span by span (not just whose paragraph it is) and lets you hide/show them, with a retroactive, word-diffed backfill from the project room's own save history. Editor-only: never read by the publish fold, never shown outside the editor |
...
### Authors mode — who wrote what, colored per span and retroactive

When more than one person is working the same draft, the Newsroom's **Authors**
toggle (🎨, next to Comments) colors each collaborator's own words — the exact
span they typed, not the whole paragraph it landed in — with a legend to
hide/show any one collaborator's text. An editor's insert can sit right in the
middle of someone else's sentence (a bracketed clarifying note, a fact-check
question) and only that inserted run picks up their color; the rest of the
sentence stays whoever wrote it. It's purely an editing aid: each run is
wrapped in a `<span data-author>` using the same safe range-wrap the citation
binder already trusts (splitting an existing span cleanly when one person's
edit lands inside another's), riding the draft's own HTML exactly like
`structure.js`'s `data-sec` (Invariant I1) — it autosaves and syncs like the
words do, but `htmlToBlocks` builds every published block from a node's TEXT
alone and never copies its attributes, however deeply the spans nest: none of
it — the colors, the tags, who hid whom — ever reaches Preview, the published
...
boundary the same way `tests/leakage-gate.mjs` guards structure.
...
past revisions, and `app/feedback/authorship.js`'s `advanceRuns` word-diffs
each paragraph between consecutive saves (the same LCS engine the
version-history diff already uses) to reconstruct which RUN of words each
save actually contributed — not just who happened to save last. A solo,
unshared draft has no such history to mine and simply starts coloring from
the next edit.
