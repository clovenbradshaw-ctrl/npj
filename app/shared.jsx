...
function SourceCard({ srcKey, onClose, pinned, quote }) {
  ...
  if (!s) return null;
  // the pinned source-span(s) for THIS claim win over the source's generic pull
  // quote — the exact words in the source that back the passage you hovered (one
  // claim can cite the same source from several spots)
  const cited = (Array.isArray(quote) ? quote : (quote ? [quote] : [])).map(s => String(s || "").trim()).filter(Boolean);
  return (
      ...
      <div style={{ padding: "10px 12px 12px" }}>
        ...
        {cited.length ? (
          <div style={{ marginBottom: 10 }}>
            <div className="np-eyebrow" style={{ color: "var(--ink-soft)", fontSize: 9.5, marginBottom: 3 }}>The cited passage{cited.length > 1 ? "s" : ""} — in the source</div>
            {cited.map((c, i) => (
              <div key={i} style={{ borderLeft: "3px solid var(--yellow-deep)", paddingLeft: 9, fontStyle: "italic",
                fontSize: 13.5, lineHeight: 1.34, color: "var(--ink)", marginBottom: i < cited.length - 1 ? 6 : 0 }}>“{c}”</div>
            ))}
          </div>
        ) : s.pull_quote && (
